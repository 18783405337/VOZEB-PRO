...
- [工作台] 图片与视频结果框按当前创作对话展示全部独立生成轮次，新建对话才清空结果；历史卡同步汇总该对话全部结果和耗时，后续输出不自动改标题；图片桌面端恢复一行 4 个并自动换行，刷新、跨轮次选择、删除和重试保持稳定记录关联。
- [Agent] 生图与视频工作台的规划和任务提交改为同一请求链收口，成功创建生成任务后不再残留“思考中”；站内永久参考图直接复用稳定 URL，服务端仍按具体上游协议转换为 URL、multipart 或 base64。
- [积分] 修复普通用户使用小数模型价格时 PostgreSQL 当日钱包更新参数被误推断为整数的问题，`1.7/1.8` 等小数扣费不再返回 500。
- [生成] 修复 Docker 和代理环境中长耗时生成被 Undici 默认 5 分钟响应超时提前中断的问题；系统代理、生成 Route、页面恢复和 Worker 生命周期统一覆盖最长视频请求及落盘余量，文本/图片/视频的原业务超时与各上游协议保持不变。
...
- [图片] 图片工作台会等待公共 Session 与模型目录完成水合后再开放生成；刷新页面时按稳定创作会话恢复最新生成记录和结果，连续主动生成仍保留在同一会话中。
- [视频] 视频工作台刷新或恢复会话时按稳定 `creativeConversationId` 恢复最新生成记录及成功、失败或生成中结果，不再只显示公开消息而让左侧结果区为空，也不会覆盖当前本地运行中的任务。
- [后台] GitHub 公开 Skill 不再因缺少 SPDX 许可证而拒绝导入，仍固定 commit、校验内容哈希并默认停用。
- [Skill] GitHub Skill 选定后由后台默认文本模型整理为中文用途、关键词、工作区和项目原生执行规则；安装命令、仓库脚本、环境变量、外部 API 与密钥示例不会直接进入表单，模型未配置或返回无效结构时明确失败。
- [支付] 表单支付改为从当前用户订单读取服务端已保存参数，在带动态 CSP 和一次性 nonce 的隔离提交页中自动提交；生产环境拒绝公网 HTTP 支付地址，Checkout 接口同步采用统一业务响应。
- [Canvas] 项目列表和素材弹窗使用服务端分页，删除、新增或越界页会重新对齐；保存请求携带最后确认版本，PostgreSQL 与文件 Provider 均拒绝旧快照，版本时间严格单调递增。
- [构建] 默认构建不再强制单 Worker 或固定 1GB/1.5GB Node 堆，按运行环境可用资源执行；部署者仍可通过 `NEXT_BUILD_CPUS` 和 `NODE_OPTIONS` 显式限制资源，发布检查继续使用独立构建目录。
- [上游] `chatgpt2api` 继续按标准 OpenAI 兼容协议请求，不新增专用协议或猜测异步查询路径；图片接口仅返回任务 ID、网页内容或无效 JSON 时保留原任务进入待确认，不切换渠道、不退款、不重复创建，Agent 会显示明确等待状态。
- [Agent] 未显式选择 Skill 时不再由规划器自动注入角色设定等普通 Skill，默认提示词优化与智能规划保持不变；离开或刷新创作页后会自动恢复最近运行中的原会话，工作台任务可直接返回对应 Agent 对话。
- [模型] Agent 公共模型目录会在后台配置变化和页面重新可见时刷新；生图工作台只展示图片模型，视频工作台只展示视频与音频模型，统一 Agent、Canvas 和短剧继续按各自能力展示。
- [界面] 套餐选择弹窗按商品数量自适应宽度，移除重复标题与无意义周期占位，压缩价格、积分、权益和操作区；两套餐不再出现大片空白，最多 8 个套餐保持桌面多列与移动端单卡切换。
- [构建] 固定 Web 与文档站的 Next.js 追踪及 Turbopack 根目录，避免仓库根工具生成临时锁文件后导致 Standalone 入口路径变化和发布检查误报失败。
- [供应链] 将 ESLint 工具链使用的 `brace-expansion` 2.x/5.x 传递依赖分别固定到已修复的 `2.1.4` 与 `5.0.9`，发布审计不再命中新披露的高危拒绝服务漏洞。
- [供应链] 将应用与 Shadcn 工具链的 Undici 升级到已修复的 `8.9.0`/`7.29.0`，并更新 `fast-uri`、`hono`、`ip-address`，发布审计不再命中新披露的缓存解析、请求走私、CRLF、SSRF 与 ReDoS 漏洞。
- [数据库] PostgreSQL 在线读取进一步收敛为实体、状态、分页和聚合查询；完整认证、提示词、生成记录与注销快照只允许备份事务使用，媒体过期清理和对象存储迁移改为有界批次处理。
- [数据] Agent 对话和 Canvas 删除改为服务端聚合硬删除：关联消息、Run、任务和生成记录不会在刷新后恢复；独占媒体随之回收，仍被素材库、其他项目、用户头像或作品引用的媒体继续保留。
...
- [支付] 上游支付表单 HTML 只在服务端解析为受控 action、method 和隐藏字段，浏览器不再通过 `document.write()` 或等价方式执行同源上游脚本。
- [备份] 后台业务 JSON 导入固定为无删除的账号配置恢复：PostgreSQL 使用表锁、事务和实体 upsert，文件 Provider 写入失败会回滚全部原文件；不完整业务包不能冒充整库灾难恢复。
- [计费] AI 消费幂等键由服务端绑定用户、业务请求、逻辑模型、渠道、上游模型和调用类型生成，客户端请求头不能控制本地流水；同一业务身份对应不同请求参数时返回 409，零积分调用继续记录并支持失败撤销。
- [生成] 生产运行时统一把跨运行时 `FormData` 转换为 Undici multipart 请求体，视频参考素材不再被错误发送成 `text/plain` 与 `[object FormData]`；视频工作台会在提交前预占并发槽，快速重复点击只创建一个任务，刷新后可继续查看或取消原任务。
- [界面] 套餐列表按最多 8 个方案优化为紧凑等高卡片，桌面多列展示，390px/430px 使用可横向选择的单卡视图，价格和购买操作不会溢出。
- [供应链] GitHub Skill 固定到不可变 commit，校验 SPDX 许可证与 SHA-256 内容哈希并默认停用；Docker 镜像仅在正式 Tag 门禁通过后发布 `latest`，同时生成 SPDX SBOM、对不可变 digest 签名并附加证明。
- [上游] 后台渠道管理取消探测、批量检测、能力验证和验证记录，不再保存检测快照；渠道接入改为五步配置，管理员在用户工作台通过真实业务请求验证文本、图片、视频和音频能力，运行时自动故障切换保持不变。
- [界面] Agent Skills 桌面卡片改为独立双列流，长规则内容不会再撑高相邻卡片；移动端保持单列。
- [工程] 根目录、CI、Web 与 Docs 统一使用 pnpm 11.9.0；新增 ESLint、Playwright E2E、Dependabot、CODEOWNERS、Issue/PR 模板和第三方许可证清单。发布检查使用唯一临时构建目录并包含审计、格式、测试、类型和生产构建，构建结束后恢复 Next 自动改写的类型配置；短剧项目页的 Header 和剧本编辑区拆为同目录职责组件，并覆盖桌面与移动端真实项目回归。
