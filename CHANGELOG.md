...
- [部署] 将 GitHub、版本检查、更新中心、Compose 默认镜像、安装向导和 GitHub Actions 统一切换到 `18783405337/VOZEB-PRO` 与 `ghcr.io/18783405337/vozeb-pro`。
- [发布] 使用不可变版本标签 `v0.0.6` 发布主应用和文档镜像，避免生产部署依赖 `latest` 漂移。

## Unreleased

- [Canvas] 修复单租户部署新建画布错误进入 SaaS 成员校验并返回 500 的问题；创建项目统一使用可信租户解析，单租户写入默认租户，SaaS 模式继续执行成员隔离。
- [数据库] 完成租户核心隔离加固：租户所有者转移同步成员角色，新用户自动加入默认租户，未知 Host 不再回退默认租户，生成记录访问强制租户作用域，并补充核心资源租户复合索引。
