...
function taskRow(overrides: Record<string, unknown> = {}) {
    ...
    return {
        ...
        reference_images: ["https://cdn.example/face-1.png", "https://cdn.example/face-2.png"],
        source_video_uri: "https://cdn.example/source.mp4",
...
function resultRow(overrides: Record<string, unknown> = {}) {
    return {
        ...
        user_id: "user-a",
        title: "Dance transfer",
        cover_uri: "https://cdn.example/face-1.png",
...
describe("ActionTransferRepository", () => {
    it("creates a task with structured provider request fields and a caller-supplied id", async () => {
        const query = vi.fn().mockResolvedValue(queryResult([taskRow()]));
        const repository = new ActionTransferRepository({ query } as unknown as QueryExecutor);
        ...
        await expect(
            repository.createTask({
                ...
                userId: "user-a",
                title: "Dance transfer",
                referenceImages: ["https://cdn.example/face-1.png", "https://cdn.example/face-2.png"],
        ...
        expect(sql).toContain("INSERT INTO action_transfer_tasks");
        expect(params.slice(0, 3)).toEqual(["task-1", "tenant-a", "user-a"]);
        expect(params).toContain(JSON.stringify(["https://cdn.example/face-1.png", "https://cdn.example/face-2.png"]));
    });
    ...
    it("returns runtime input only inside the owner scope", async () => {
        const query = vi.fn().mockResolvedValue(queryResult([taskRow()]));
        const repository = new ActionTransferRepository({ query } as unknown as QueryExecutor);
    ...
    it("isolates runtime updates and clamps progress", async () => {
    ...
    it("idempotently writes a same-id result and completes only the owner task", async () => {
        ...
        const [sql, params] = query.mock.calls[0] as [string, unknown[]];
        expectOwnerScope(sql);
        expect(sql).toContain("INSERT INTO action_transfer_results");
        expect(sql).toContain("SELECT id, tenant_id, id, user_id");
        expect(sql).toContain("ON CONFLICT (id) DO UPDATE");
        expect(params.slice(0, 3)).toEqual(["tenant-a", "user-a", "task-1"]);
    });

    it("fails only the owner task and bounds the stored error", async () => {
    ...
    it("lists results only inside the owner scope", async () => {
