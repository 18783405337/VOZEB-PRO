        ...
        const payload = await requestSpecializedProvider(
            {
                ...
                body: compact({
                    ...
                    prompt: text(params.prompt) || context.specializedConfig?.klingPrompt,
                    mode: context.specializedConfig?.klingMode || (params.mode === "pro" ? "pro" : "std"),
                    external_task_id: request.localTaskId,
                    callback_url: text(params.callback_url) || context.specializedConfig?.klingCallbackUrl,
                    watermark_info: context.specializedConfig?.klingWatermarkEnabled === undefined ? booleanRecord(params.watermark_info) : { enabled: context.specializedConfig.klingWatermarkEnabled },
                }),
