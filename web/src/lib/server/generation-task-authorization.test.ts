...
describe("generation upstream task authorization", () => {
    ...
    it("requires an exact owner, channel, task type and upstream model match", async () => {
        ...
        await expect(userOwnsGenerationUpstreamTask({ tenantId: "tenant-one", userId: "user", capability: "video", channelId: "channel", upstreamModel: "vendor-video", upstreamTaskId: "task" })).resolves.toBe(true);
        expect(mocks.getByUpstream).toHaveBeenCalledWith("video", "tenant-one", "user", "channel", "task");

        mocks.getByUpstream.mockResolvedValue({ status: "running", payload: { config: { model: "other" } } });
        await expect(userOwnsGenerationUpstreamTask({ tenantId: "tenant-one", userId: "user", capability: "video", channelId: "channel", upstreamModel: "vendor-video", upstreamTaskId: "task" })).resolves.toBe(false);
    });
