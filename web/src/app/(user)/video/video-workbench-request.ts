...
export function buildVideoWorkbenchRequest(input: BuildVideoWorkbenchRequestInput): VideoWorkbenchRequestResult {
    ...
    if (!text) return { snapshot: null, issue: "missing-prompt", message: "请输入视频提示词" };
    ...
    if (!input.isAiConfigReady(requestConfig, requestModel)) return { snapshot: null, issue: "config-unavailable", message: "请联系管理员在后台配置可用视频模型" };
