...
import { CircleDot, Eraser, FolderOpen, Globe2, Grid2x2, Hand, Image as ImageIcon, Info, LayoutGrid, Moon, Music2, Palette, PenTool, Redo2, Settings2, Square, Sun, Trash2, Type, Undo2, Upload, Video } from "lucide-react";
    ...
    return (
        <div className="canvas-toolbar-dock-wrap pointer-events-none absolute bottom-5 left-0 right-0 z-50 flex justify-center">
            ...
            >
                ...
                {onAddDrawing && (
                    <ToolbarButton id="tool-drawing" label="绘图" hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddDrawing}>
                        <PenTool className="size-4.5" />
                    </ToolbarButton>
                )}
                {onAddFrame && (
                    <ToolbarButton id="tool-frame" label="框架" hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddFrame}>
...
function toolLabel(id: string) {
    ...
    if (id === "tool-drawing") return "绘图";
    if (id === "tool-frame") return "框架";
