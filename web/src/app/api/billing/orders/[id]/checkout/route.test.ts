...
describe("payment checkout route", () => {
    ...
    it("returns the stable 409 envelope and records the internal merchant reason", async () => {
        mocks.createCheckout.mockRejectedValueOnce(new BillingInputError("Merchant account is not configured.", 409, "MERCHANT_ACCOUNT_NOT_CONFIGURED"));

        const request = new NextRequest("http://localhost/api/billing/orders/order-one/checkout", { method: "POST", body: JSON.stringify({ collectionMode: "tenant" }) });
        const response = await POST(request, { params: Promise.resolve({ id: "order-one" }) });

        expect(response.status).toBe(409);
        expect(await response.json()).toEqual({ code: 409, data: null, msg: "Merchant account is not configured.", error: "Merchant account is not configured." });
        expect(mocks.recordAudit).toHaveBeenLastCalledWith(expect.objectContaining({ status: "failure", metadata: expect.objectContaining({ reason: "MERCHANT_ACCOUNT_NOT_CONFIGURED" }) }));
    });
});
