...
export async function POST(request: Request) {
    ...
    try {
        ...
        const result = await importAgentSkillFromGithub({ url: body.url, path: typeof body.path === "string" ? body.path : undefined });
        return apiSuccess(result, result.skill ? "Skill 提取成功" : "请选择要提取的 Skill");
    } catch (error) {
