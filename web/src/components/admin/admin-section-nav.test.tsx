...
describe("admin navigation order", () => {
    it("keeps the existing business classifications and generation operations entry", () => {
        expect(adminSectionGroups[0]?.items.map((item) => item.key)).toEqual(["apps"]);
        expect(adminSectionGroups.slice(1).map((group) => group.title)).toEqual(["经营分析", "商品运营", "营销推广", "财务管理", "上游配置", "系统管理", "存储与备份", "内容运营", "帮助与支持"]);
        ...
        expect(adminSectionGroups.find((group) => group.title === "系统管理")?.items.map((item) => item.label)).toEqual(["租户管理", "站点资料", "基础设置", "注销申请"]);
